...
const CreateMedicine: React.FC = () => {
  ...
  const onDrop = useCallback(
    (
      ...
      let filesArray = acceptedFiles;
      if (filesArray.length > 10) {
        toast.warning("Bạn chỉ có thể tải lên tối đa 10 file cùng lúc");
  ...
  const validationSchema = Yup.object({
    ...
    unitDetails: Yup.array()
      .of(
        Yup.object().shape({
          unitId: Yup.number().required("Đơn vị là bắt buộc"),
  ...
  const handleImageChange = (
    ...
    if (event.currentTarget.files) {
      let filesArray = Array.from(event.currentTarget.files);
  ...
  const getDropzoneConfig = (setFieldValue: any) => {
    return {
      accept: {
        "image/*": [".jpeg", ".png", ".jpg"],
      },
      ...
      onDrop: (acceptedFiles: File[]) =>
        onDrop(acceptedFiles, null, null, setFieldValue),
      onDropRejected: (fileRejections: any) => {
        toast.error("Có lỗi khi tải file. Vui lòng chỉ chọn file hình ảnh.");
      },
    };
  };

  // Sử dụng trong component render
  const ImageDropzone = ({ setFieldValue, errors, touched }: any) => {
    const { getRootProps, getInputProps, isDragActive } = useDropzone(
      getDropzoneConfig(setFieldValue)
    );
    ...
    return (
      <>
        ...
        >
          ...
          <em>
            (Tối đa 10 hình ảnh, chỉ chấp nhận định dạng .png, .jpg, .jpeg)
          </em>
          <div className="mt-2">
            <i
